...
namespace UdemyNewMicroservice.Shared
{
    ...
    public class ServiceResult<T> : ServiceResult
    {
        ...
        public new static ServiceResult<T> Error(string title, string description, HttpStatusCode status)
        {
            return new ServiceResult<T>
            {
                ...
                Fail = new ProblemDetails()
                {
                    Title = title,
                    Detail = description,
                    Status = status.GetHashCode() //int karşılığını verir bu şekilde de , cast de edebilir.
                }
            };
        }
        ...
        //Poliformizim desteklemediğimiz için new ile yukarıdakiler kullanılmazsa aşağıdakiler kullanılsın mantığı yapmış olduk.
    }
}